...
import { Check, Copy, Pencil, Plus, Power } from "lucide-react";
...
export function LinksPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3">{links.data?.items.map((item) => <LinkRow key={item.shortcode} item={item} folders={folders.data ?? []} />)}</div>
...
function LinkRow({ item, folders }: { item: LinkItem; folders: Array<{ id: number; name: string }> }) {
  ...
  const save = () => { void mutation.mutateAsync({ label, folder_id: item.folder_id }).then(() => setEditing(false)); };
  return <Card className="grid gap-4 md:grid-cols-[minmax(0,1fr)_110px_150px_210px] md:items-center"><div className="min-w-0 space-y-1"><p className="m-0 truncate font-medium text-accent">{item.shortcode}</p>{editing ? <Input value={label} onChange={(event) => setLabel(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") save(); }} /> : <p className="m-0 text-sm">{item.label || "Без названия"}</p>}<p className="m-0 truncate text-xs text-subtle">{item.url}</p></div><div className="text-sm text-subtle">{item.access_count} переходов</div><div className="text-sm text-subtle">{formatDate(item.created_at)}<br />{folderName}</div><div className="flex items-center justify-end gap-1"><button className="pill" type="button" title="Копировать короткую ссылку" aria-label="Копировать короткую ссылку" onClick={() => void copyToClipboard(item.short_url)}><Copy className="h-4 w-4" /></button><button className="pill" type="button" title={editing ? "Сохранить" : "Редактировать название"} aria-label={editing ? "Сохранить" : "Редактировать название"} onClick={editing ? save : () => setEditing(true)}>{editing ? <Check className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}</button><AlertDialog.Root><AlertDialog.Trigger asChild><button className="pill" type="button" title={item.is_active ? "Отключить ссылку" : "Включить ссылку"} aria-label={item.is_active ? "Отключить ссылку" : "Включить ссылку"}><Power className="h-4 w-4" /></button></AlertDialog.Trigger><AlertDialog.Portal><AlertDialog.Overlay className="fixed inset-0 bg-black/50" /><AlertDialog.Content className="fixed left-1/2 top-1/2 w-[min(92vw,400px)] -translate-x-1/2 -translate-y-1/2 rounded-panel border border-border bg-panel p-6"><AlertDialog.Title className="text-lg font-semibold">{item.is_active ? "Отключить ссылку?" : "Включить ссылку?"}</AlertDialog.Title><AlertDialog.Description className="mt-2 text-sm text-subtle">URL и shortcode останутся навсегда неизменными. {item.is_active ? "Отключённая ссылка будет отвечать 410." : "Переходы снова будут направляться на прежний URL."}</AlertDialog.Description><div className="mt-5 flex gap-2"><AlertDialog.Cancel asChild><Button variant="secondary">Отмена</Button></AlertDialog.Cancel><AlertDialog.Action asChild><Button onClick={() => void mutation.mutateAsync({ is_active: !item.is_active })}>{item.is_active ? "Отключить" : "Включить"}</Button></AlertDialog.Action></div></AlertDialog.Content></AlertDialog.Portal></AlertDialog.Root><Switch.Root checked={item.is_active} onCheckedChange={(checked) => mutation.mutate({ is_active: checked })} className="relative h-6 w-11 rounded-full bg-border data-[state=checked]:bg-accent"><Switch.Thumb className="block h-5 w-5 translate-x-0.5 rounded-full bg-white transition data-[state=checked]:translate-x-[22px]" /></Switch.Root></div></Card>;
}
