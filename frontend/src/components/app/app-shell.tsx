...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const renderLink = (to: string, label: string, Icon: typeof Home) => <NavLink key={to} to={to} onClick={onNavigate} className={({ isActive }) => cn("flex items-center gap-3 rounded-panel px-3 py-2 text-sm text-subtle transition hover:bg-muted hover:text-text", isActive && "bg-accent/10 text-accent")}><Icon className="h-4 w-4" /><span>{label}</span></NavLink>;
